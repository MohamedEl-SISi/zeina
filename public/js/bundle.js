...
window.onload = function () {
  ...
  _functions2.default.expand_and_collapse('.header__burger', '.side-menu', '.app-box', 'header', 'active');
  openSearchWindow();
  _functions2.default.navExplandAndCollapse('.side-menu__expand', '.side-menu__sub-links');
  ...
  if (_helper2.default.page_type() == "single" || _helper2.default.page_type() == "quiz") {
    ...
    if (_helper2.default.page_type() == "quiz") {
      // let _answer = document.querySelector('.single-wrapper__result');
      // _answer ? _answer.scrollIntoView() : 0 ;
      _functions2.default.quiz();
    }
  ...
  if (_helper2.default.page_type() == "articles") {
    floatSide();
  }
...
functions.expand_and_collapse = function (el_1, el_2, el_3, el_4, class_1) {
	el_1 = document.querySelector(el_1);
	el_2 = document.querySelector(el_2);
	el_3 = document.querySelector(el_3);
	el_4 = document.querySelector(el_4);
	if (el_1) {

		el_1.onclick = function () {
			el_1.classList.toggle(class_1);
			el_2.classList.toggle(class_1);
			el_3.classList.toggle(class_1);
			el_4.classList.toggle(class_1);
			// document.querySelector(el_4).classList.toggle(class_1);
			if (el_1.classList.contains("active")) {
		...
		// 	el_1.classList.remove(class_1)
		// 	el_2.classList.remove(class_1)
		// 	el_3.classList.remove(class_1)
		// 	el_4.classList.remove(class_1)
		// 	if (el_1.classList.contains("active")) {
		// 		document.getElementsByTagName("html")[0].style.overflow = "hidden";
...
functions.navExplandAndCollapse = function (el_arrow, el_dropdown) {
	...
	var _loop = function _loop(i) {
		arrow[i].onclick = function () {
			...
			// 	dropDown[x].classList.remove('active')
			// 	arrow[x].classList.remove('active')
			// }
			//let activated = document.querySelectorAll('.mainNav__menu li .drop-down-box.active')
			// if(activated){
			// 	for(let x=0;x<activated.length;x++){
			// 		activated[x].classList.remove('active')
			// 		arrow[x].classList.remove('active')
			// 	}
			// }
	...
	// let arrowActive = document.querySelectorAll('.mainNav__menu li .arrow.active')[0]
	// let dropDownActive = document.querySelectorAll('.mainNav__menu li .drop-down-box.active')[0]
	// if(arrowActive){
	// 	arrowActive.onclick = () => {
...
functions.quiz = function () {
	...
	var count = 0,
	    ...
	    min_score = void 0,
	    max_score = void 0,
	    q_box = document.querySelector(".q-box"),
	    questions = document.querySelectorAll(".question-head"),
	    _choise = document.querySelectorAll(".choise"),
	    _answer = document.querySelectorAll(".single-wrapper__answer");
	// ans_value = _choise.getAttribute("value");

	var _loop3 = function _loop3(z) {

		_choise[z].addEventListener("click", function () {
			to_num = Number("" + _choise[z].getAttribute("value"));
			count += to_num;
			// console.log(count)
			// _choise[z].parentNode.parentNode.parentNode.classList.add("hide")
			if (screen.width < 768) {
				q_box.scroll({
					top: scrl_mob,
					left: 0,
			...
			for (var i = 0; i < _answer.length; i++) {
				min_score = Number("" + _answer[i].getAttribute("minscore"));
				max_score = Number("" + _answer[i].getAttribute("maxscore"));

				if (count > min_score && count <= max_score) {
					// console.log(min_score , max_score)
					for (var x = 0; x < _answer.length; x++) {
						_answer[x].classList.remove("show");
						_answer[i].classList.add("show");
					}
				}
			}
		});
	...
	for (var z = 0; z < _choise.length; z++) {
		_loop3(z);
	}
};
...
// let base_url2 = "https://zeinaforher.com";
// let base_url = "https://cors-anywhere.herokuapp.com/https://zeinaforher.com";
...
loadmore.loadCards = function () {
  var _row = _helper2.default.selector_query(".cont"),
      ...
      _content = "",
      params_fetcher = function params_fetcher(url, param) {
    var params = new URL(url);
    return params.searchParams.get(param);
  },
      spread_url = function spread_url() {
    var pathArray = window.location.pathname.split('/');
    if (section_name == "news" || section_name == "tags" || section_name == "articles") {
      return base_url + "/api/" + decodeURIComponent(pathArray[1]) + "/" + decodeURIComponent(pathArray[2]) + "?page=" + offset;
      // return `${base_url}/api/news/جمالك?page=${offset}` 
      // return `${base_url}/api/articles/بعيون-آدم?page=${offset}` 
    }

    if (_helper2.default.page_type() == "search") {
      // console.log(`${base_url}/api/${decodeURIComponent(pathArray[1])}/${params_fetcher(window.location.href , "q")}`)
      return base_url + "/api/" + decodeURIComponent(pathArray[1]) + "?q=" + params_fetcher(window.location.href, "q") + "&page=" + offset;
      // return `${base_url}/api/search?q=ت&page=${offset}`
    }
    if (_helper2.default.page_type() == "quiz") {
      // return `${base_url}/api/${decodeURIComponent(pathArray[1])}`
      return base_url + "/api/exams";
    }
    if (_helper2.default.page_type() == "files") {
      return base_url + "/api/" + decodeURIComponent(pathArray[1]);
    ...
    var pathArray = window.location.pathname.split('/');
    return "" + decodeURIComponent(pathArray[1]);
  };
  // spread_url();
  _btn.onclick = function () {
    _helper2.default.add_loader(_row);
    _btn.style.visibility = "hidden";
    // if (section_name == "news" || section_name == "search" || section_name == "files" || section_name == "tags" || section_name == "quiz") {
    _content = "";
    _axios2.default.get(spread_url()).then(function (res) {
      ...
      if (section_name == "articles") {
        for (var i = 0; i < res.data.data.data.length; i++) {
          _content += "\n                  <div class=\"col-lg-6 col-md-6 col-sm-12 col-xs-12\">\n          <div class=\"news-card news-card--card news-card--switch profile-card profile-card--circle\">\n            <a class=\"news-card__link\" href=\"" + base_url + "/articles/" + res.data.data.data[i].id + "/" + res.data.data.data[i].slug + "\"></a>\n            <div class=\"news-card__img\">\n              <img class=\"lazy\" src=\"" + res.data.data.data[i].imageUrl + "\" image-src=\"https://zeinaforher.com/images/120x90/2020/8/\u0631\u0633\u0627\u0644\u0629-\u0625\u0644\u0649-\u0643\u0644-\u0631\u0627\u0626\u062F\u0629-\u0623\u0639\u0645\u0627\u0644-\u0623\u0646\u062A\u0650-\u0627\u0644\u0645\u0633\u062A\u0642\u0628\u0644_1597424662.jpg?1600535982\" alt=\"\u0631\u0633\u0627\u0644\u0629 \u0625\u0644\u0649 \u0643\u0644 \u0631\u0627\u0626\u062F\u0629 \u0623\u0639\u0645\u0627\u0644.. \u0623\u0646\u062A\u0650 \u0627\u0644\u0645\u0633\u062A\u0642\u0628\u0644\">\n            </div>\n            <span class=\"profile-name\">" + res.data.data.data[i].publisher_name + "</span>\n            <div class=\"news-card__caption\">\n              <div class=\"news-card__details\">  \n                <div class=\"news-card__caption-row\">\n                </div><span class=\"news-card__title\">" + res.data.data.data[i].title + "</span>\n              </div>\n            </div>\n                      </div>\n                    </div>";
        }
      } else {
        for (var _i = 0; _i < res.data.data.data.length; _i++) {
          _content += "\n                  <div class=\"col-lg-12 col-md-12 col-sm-12 col-xs-12\">\n                    <div class=\"news-card news-card--card news-card--switch section-theme " + (section_name == "quiz" || section_name == "files" ? "quiz-card" : "") + "\"><a class=\"news-card__link\" href=\"" + base_url + "/news/" + res.data.data.data[_i].id + "/" + res.data.data.data[_i].slug + "\"></a>\n                      <div class=\"news-card__img\"><img class=\"lazy\" src=\"" + res.data.data.data[_i].imageUrl + "\" image-src=\"\">\n                      </div>\n                      <div class=\"news-card__caption-row\">";
          if (section_name == "quiz") {
            _content += "<a class=\"cat\" href=\"" + location.origin + "/" + get_url_data() + "/" + res.data.data.data[_i].section.slug + "\">" + res.data.data.data[_i].section.name + "</a>";
            // _content +=``
          } else if (section_name == "files") {
            _content += "";
          } else {
            _content += "<a class=\"cat\" href=\"" + location.origin + "/" + get_url_data() + "/" + res.data.data.data[_i].section.slug + "\">" + res.data.data.data[_i].section.name + "</a>\n                          ";
          }
          _content += "\n                      </div><div class=\"news-card__caption\">\n                        <div class=\"news-card__details\">\n                          <div class=\"news-card__caption-row\">\n                          </div><span class=\"news-card__title\">" + res.data.data.data[_i].title + "</span>\n                          <div class=\"news-card__info\">\n                            <p>" + (res.data.data.data[_i].desc == null ? "" : res.data.data.data[_i].desc) + "</p>\n                          </div>\n                        </div>\n                      </div>\n                    </div>\n                  </div>\n                  ";
        }
      }
    ...
    // } ///////// if statment check section type
