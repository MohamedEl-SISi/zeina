...
<div class="layout-wrap">
  <section class="main-section">
    <div class="breadCrumb mt15">
      <div class="breadCrumb__list">
        ...
        <li class="breadCrumb__item">
          ...
          <li class="breadCrumb__item">
            <a class="breadCrumb__link" href="{{url('articles')}}">آراء ومشاركات</a>
          </li>
        ...
        <li class="breadCrumb__item">
    x      <span class="breadCrumb__link"> {{$item->section->name}}  </span>
        </li>
        @endif
    ...
    <div class="single-wrapper" id="single-wrapper" article-id="253151">
      <div class="single-wrapper__title">
        <div class="single-wrapper__writter">
          <div class="single-wrapper__writter-img">
            @if(is_null($item->image))
            <img src="{{url('default.png')}}" alt="{{$item->title}}"/>
            @else
            <img src="{{$Helper->getImageUrl($item->image->fileName,'600x400',$item->image->path)}}?{{time()}}" alt="{{$item->title}}" />
            @endif

          </div>
          <span class="single-wrapper__writter-name">{{$item->publisher_name}}</span>
        </div>
        <h1>{{$item->title}}</h1>
      </div>
  ...
  @if(!IS_MOBILE)
    @include('website.sections.asideArticles')
  @endif
</div>
