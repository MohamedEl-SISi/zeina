...
@php
  ...
  if(is_null($item->firstNews?$item->firstNews->relatedNews->image:null))
  {
       $imageUrl = url('default.png');
  }else
  {
      $imageUrl= $item->firstNews->relatedNews->image->getImageThumbAttribute().'?'.time();
  }
@endphp

<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
  <div class="news-card news-card--card news-card--switch section-theme quiz-card">
    <a class="news-card__link" href="{{url('files/'.$item->id.'/'.$item->slug)}}"></a>
